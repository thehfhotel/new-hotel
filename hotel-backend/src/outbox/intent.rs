...
pub enum WritebackIntent {
    ...
    /// Phase 2 (check-in registration) — mirror a captured guest document
    /// (Thai ID card / passport / face photo) into the legacy `Tb_Save_Image`
    /// table so iHOTEL's registration screen shows the same photo.
    ///
    /// The recipe INSERTs a **provisional** row (`cust_no`/`cin_no` empty),
    /// keyed only by `tmp_no`; the existing check-in writeback's
    /// `UPDATE Tb_Save_Image SET cin_no=…, cust_no=… WHERE tmp_no=<tmp_no>`
    /// (walk-in recipe stmt 1b) later stamps the identifiers — the same
    /// two-step the legacy .NET app uses (photo saved before the check-in row
    /// exists). The image bytes are NOT carried in the payload: the writeback
    /// worker's resolver loads `doc_image` (bytea) from `ht_guest_documents`
    /// keyed on `doc_id` into `ResolvedJob::guest_document_image` (recipes stay
    /// PG-pure). Ledgered — `Tb_Save_Image.id` is IDENTITY, so a
    /// crash-after-commit retry would mint a duplicate photo row.
    ///
    /// **Shipped DARK** behind `GUEST_DOCUMENT_STORAGE_ENABLED` (the emitter is
    /// gated; this variant + recipe are always compiled/routable).
    MirrorGuestImage {
        /// `ht_guest_documents.doc_id` — the canonical PK. Drives the resolver's
        /// `doc_image` bytea load and the deterministic aggregate id.
        doc_id: i64,
        /// Canonical `doc_type` (`thai_id_card` | `passport` | `face_photo`).
        /// The recipe maps it to the legacy `Tb_Save_Image.ttype` Thai literal.
        doc_type: String,
        /// Legacy `HT_Customers.Cust_no` if already known (diagnostic /
        /// future non-provisional path). The provisional INSERT writes `''`.
        #[serde(default)]
        cust_legacy_no: Option<String>,
        /// Legacy `HT_CheckIn_H.Cin_no` if already known (diagnostic /
        /// future non-provisional path). The provisional INSERT writes `''`.
        #[serde(default)]
        cin_legacy_no: Option<String>,
        /// Provisional `Tb_Save_Image.tmp_no` — the linkage key the check-in
        /// writeback matches on. Generated by the route, stored on the
        /// canonical row, and passed as `photoTmpNo` on the check-in POST.
        tmp_no: String,
    },

    /// Phase 4 (TM.30 companion guests) — mirror a companion guest added to a
    /// folio into the legacy `HT_CheckIn_Other_People` table (name + country
    /// only; the legacy table has no id-card column). Mirrors the walk-in
    /// recipe's TM.30 primary-guest INSERT (stmt N+1) — the `Cin_name` carries
    /// the country-derived prefix (`guest_prefix_for_country`).
    ///
    /// Carries `cin_legacy_no` directly (resolved by the route from
    /// `ht_checkins.legacy_cin_no` before enqueue — the intent is only emitted
    /// once the folio has a legacy `Cin_no`), so no `ResolvedJob` lookup —
    /// mirrors the `UpdateCustomer` / `UpdateRoom` payload-carries-the-key
    /// shape. Ledgered — `HT_CheckIn_Other_People.id` is IDENTITY, so a
    /// crash-after-commit retry would mint a duplicate companion row.
    ///
    /// **Shipped DARK** behind `TM30_COMPANION_WRITEBACK_ENABLED`.
    MirrorCompanion {
        /// Legacy `HT_CheckIn_H.Cin_no` this companion belongs to. Required —
        /// the recipe errors on empty.
        cin_legacy_no: String,
        /// Companion display name (first + last). The recipe prefixes it per
        /// `guest_prefix_for_country(country)` before writing `Cin_name`.
        name: String,
        /// Companion country / nationality → `Cin_contry` verbatim.
        #[serde(default)]
        country: String,
    },
...
pub fn round_aggregate(site_id: &str, legacy_round_id: i32) -> Uuid {
    ...
    Uuid::new_v5(
        &namespace,
        format!("{site_id}:{legacy_round_id}").as_bytes(),
    )
}
    ...
    pub fn aggregate_id(&self) -> Uuid {
        match self {
            ...
            WritebackIntent::AdjustProductStock {
                ...
                ..
            } => product_aggregate_id.unwrap_or_else(|| product_aggregate_fallback(prod_legacy_no)),
            ...
            // Deterministic per-document / per-folio aggregate ids so jobs for
            // the same document (or the same folio's companions) group in the
            // `writeback_jobs` index — the canonical PG SERIALs are not portable
...
/// Deterministic per-document aggregate id for [`WritebackIntent::MirrorGuestImage`].
/// v5 UUID over `ht_guest_documents.doc_id` under a stable namespace so the
/// `writeback_jobs.aggregate_id` index groups a document's (rare) retries.
pub fn guest_document_aggregate(doc_id: i64) -> Uuid {
    let namespace = Uuid::new_v5(&Uuid::NAMESPACE_OID, b"new-hotel.aggregate.guest_document");
    Uuid::new_v5(&namespace, doc_id.to_string().as_bytes())
}

/// Deterministic per-folio aggregate id for [`WritebackIntent::MirrorCompanion`].
/// v5 UUID over the legacy `Cin_no` so every companion added to the same folio
/// groups together in the `writeback_jobs` index. The idempotency key adds a
/// per-companion discriminator (see the create-guest route) so distinct
/// companions on one folio still enqueue distinct jobs.
pub fn companion_aggregate(cin_legacy_no: &str) -> Uuid {
    let namespace = Uuid::new_v5(&Uuid::NAMESPACE_OID, b"new-hotel.aggregate.companion");
    Uuid::new_v5(&namespace, cin_legacy_no.as_bytes())
}
...
mod tests {
    ...
    fn create_check_in_payload_roundtrips_multi_room_slice() {
        ...
        let payload = CreateCheckInPayload {
            ...
            stay: crate::domain::shared::DateRange {
                start: chrono::Utc.with_ymd_and_hms(2026, 4, 24, 3, 1, 11).unwrap(),
                    ...
                    .with_ymd_and_hms(2026, 4, 25, 4, 59, 59)
                    .unwrap(),
            },
        ...
        let parsed: CreateCheckInPayload = serde_json::from_str(&json).expect("must round-trip");
    ...
    fn update_room_payload_roundtrips_partial_update() {
        ...
        let json = serde_json::to_string(&payload).expect("must serialize");
        let parsed: UpdateRoomPayload = serde_json::from_str(&json).expect("must round-trip");
