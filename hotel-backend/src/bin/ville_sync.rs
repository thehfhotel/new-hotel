...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mssql_config = MssqlConfig {
        ...
        password: env::var("MSSQL_PASSWORD").unwrap_or_else(|_| "REDACTED-sa-pw".to_string()),
    };
    ...
    let pg_password = env::var("PG_PASSWORD").unwrap_or_else(|_| "REDACTED-sa-pw".to_string());
    ...
    let push_pool: Option<PgPool> = match env::var("PUSH_PG_SERVER") {
        Ok(push_server) => {
            ...
            let push_password = env::var("PUSH_PG_PASSWORD").unwrap_or_else(|_| "REDACTED-sa-pw".to_string());
