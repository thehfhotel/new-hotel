...
|---|---|---|---|
| 1 | PG topology | **Per-DB**: one PG cluster, two databases (`hotelnew` + new `hotelville`) | `migrations/pg/013_legacy_ct_state.sql` §"CREATE TABLE IF NOT EXISTS legacy_ct_state" "PRIMARY KEY DEFAULT 1 CHECK (id = 1)" enforces single-row by design. Per-DB inherits this naturally; per-schema or site-column adds weeks of schema rewrites. Each site gets its own row/copy for free. Backups per site without filtering. Receptionist data never co-mingled. |
...
| Password fallback `"REDACTED-sa-pw"` in `config.rs:34` | #68 |
