...
**Captured write shapes** (`docs/legacy-app/COMPAT_CHEATSHEET.md` §"Table: `HT_Round_Bill`" "**Schema** (6 cols)";
`docs/legacy-app/COMPAT_CHEATSHEET.md` §"3.20 Open Round-Bill"; `docs/legacy-app/COMPAT_CHEATSHEET.md` §"3.21 Close Round-Bill";
from `FrmDueBill.cs:1653/1670`):
...
- **J7b — SHIPPED.** `GET /api/shifts/{id}/report` (branch-aware): mirrors iHOTEL's `View_RBill_H` field-for-field — cash **received** (`>0`) vs **paid-out** (`<0`), credit (`>0`), transfer (`>0`), **deposits received** (`cr_room_in` in window) / **returned** (`cr_dep_returned_at` in window, from `ht_checkin_rooms`), grand total (`opening + net cash + credit + transfer` = iHOTEL `FrmDueBill` Labeltotal) — plus our extras (free/web, room/product sales, cash-drawer reconciliation). Window `>= round_start` (open, live preview) or `BETWEEN start AND end` (closed). **Parity verified** against prod `View_RBill_H` (round 4778: cash 2797.20 ≡ 2797.20).
