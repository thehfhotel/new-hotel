...
| `ht_checkins` | `HT_CheckIn_H` (header) | `1:1` AT HEADER LEVEL | shared | `sync/mappers/checkin.rs::apply_checkin_aggregate` | `routes/rooms.rs`, `routes/stats.rs`, calendar | walkin/checkin_to_booking recipes | ⚠️ Track B1 (migration 043) introduced `ht_checkin_rooms` junction; Track B2 (2026-05-13) rewrote the mapper to emit per-room rows there. `cin_room_id` column is **DEPRECATED — use `ht_checkin_rooms` junction instead**; left in place until Track B5 backfill completes. Mapper is multi-room as of B2; readers still single-room until B3; writeback still single-room until B4 |
| `ht_checkin_rooms` | `HT_CheckIn_Ds` | `N:1` (junction) | PG canonical | `sync/mappers/checkin.rs::apply_checkin_aggregate` (Track B2 — emits one row per `HT_CheckIn_Ds` row in the aggregate, UPSERT keyed on `(cr_cin_id, cr_room_id)`, dropped-room DELETE cleanup) | B3 follow-on (currently none — `routes/rooms.rs`, `routes/stats.rs`, calendar still join on `cin_room_id`) | B4 follow-on (currently none — walkin recipe still emits one Ds row) | ✅ Track B1 / T1 CRIT-1 (migration 043) + Track B2 / T2 CRIT-1 (mapper rewrite). Schema keyed on `(cr_cin_id, cr_room_id)` UNIQUE. Operational columns mirror the legacy `Ds` row: `cr_room_in/out`, `cr_room_status`, `cr_rate_per_night`, `cr_nights`, `cr_room_total`, deposit fields (`cr_dep_*`), `cr_cupon_count`, `cr_note`, `cr_legacy_ds_id` |
