...
| `ht_shifts` | `HT_Round_Bill` | `1:1` | PG canonical | none (Track G follow-up) | `routes/new_shifts.rs` | gate at `service/payment.rs::record_payment` + `service/checkin.rs::check_out` (round-bill — Track G9 / migration 048 stamps `ht_checkins.cin_round_bill_shift_id`) | Track F2 / T1 HIGH-5 — migration 040 adds the canonical table + one-open-per-site partial UNIQUE index. PG is source-of-truth for now; legacy `HT_Round_Bill` mirror writeback + CT sync deferred to a follow-up. Payment gate (F2) + round-bill gate (G9, migration 048) both active: each refuses unless `current_open_shift().is_some()`. G9 also adds `ht_checkins.cin_round_bill_shift_id BIGINT REFERENCES ht_shifts(shift_id)` so per-shift revenue attribution lives in canonical PG (legacy `HT_Round_Bill` / `HT_Receipt_H` have no linking column — see `docs/legacy-app/COMPAT_CHEATSHEET.md`) |
