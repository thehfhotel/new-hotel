...
| [`ROOM_GRID_REFRESH.md`](ROOM_GRID_REFRESH.md) | `FormRoomMain` refresh mechanics — timer interval + self-reschedule, the `MSSQL.CodeErr` focus gate (iHOTEL never auto-refreshes while unfocused), the "10s poll" myth correction, what a refresh actually writes, the manual Refresh button's selection-destroying side effect, and the exhaustive negative finding that no external IPC surface exists to trigger a refresh. Backs `docs/adr/0006-legacy-stale-notification.md`. |
