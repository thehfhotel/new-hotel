...
| `LOYALTY_WRITEBACK_STALL_ALERT_MINUTES` (global only) | Track F5 loyalty writeback-leg tripwire (`docs/runbooks/writeback-leg-degraded.md`): age at which a `book_channel='loyalty'` booking's writeback job still not at `done` counts as a CONFIRMED stall. Bound into `make_interval(mins => $1)`, so `int` only. Zero/negative/garbage falls back to the default rather than alerting on every job the instant it is enqueued. | `10` (min) | Rarely. The floor is what the worker does unaided — NOTIFY is sub-second, the poll fallback 30s, retry backoff and the janitor's stuck-claim steal both settle inside 5 min, so **below ~6 min you page on healthy retries**. The ceiling is the 2h hold TTL, which the alert must land well inside. |
| `LOYALTY_WRITEBACK_STALL_COOLDOWN_MINUTES` (global only) | Minimum gap between repeats of the F5 stall alert for one site. Burns only on a confirmed delivery (defect A3 — a webhook outage must not silence a degraded leg). | `30` (min) | Deliberately far shorter than `LEVEL_DRIFT_COOLDOWN_HOURS`: the condition is bounded by a 2h TTL, so a 24h window would collapse the whole incident into one message. 30 min gives an unattended overnight outage ~4 reminders inside a hold's life. |
...
| `:satellite_antenna: Loyalty-channel writeback stalled` / `:white_check_mark: ... RECOVERED` (Track F5, `scheduler::sync`) | A booking with `book_channel='loyalty'` committed to PostgreSQL but its writeback job has been in a non-`done` state for more than `LOYALTY_WRITEBACK_STALL_ALERT_MINUTES` (default 10). iHOTEL therefore does NOT show the room as `จอง` and the desk can double-book it — the `booking_create` recipe's `HT_Room_Status` row is what puts a hold on the room board (`docs/loyalty-channel.md` §"Dual-write policy for holds"). Exists because a loyalty hold dies at a **2h TTL** and every other detector here is slower or blind to a single row: the `level_drift_alert` digest needs 4h (> the TTL), the Section 9 burst alert 50 rows/hr, the queue-depth janitor 500 pending. Runs in the **backend/scheduler** container on pure canonical PG, NOT in `bin/writeback` — the likeliest cause is that the writeback worker is down, and a detector inside it would share its fate. `in_progress` and `exhausted` count as stalls (`status <> 'done'`): a worker dying mid-claim leaves the row claimed forever. Non-loyalty writebacks are ignored however old. Warning tier, no `<!channel>`. Pairs with a `:white_check_mark:` all-clear that also clears the cooldown. | **`docs/runbooks/writeback-leg-degraded.md`** — check `docker compose ps` for the `writeback` / `writeback-hfville` service FIRST (restart-cap trap: silence after a refusal page is not recovery), then the worker's startup-probe refusals, then HF Ville's WG tunnel (`wg-quick up hfville`). Never hand-write the missing legacy rows (byte-parity) and never re-enqueue the jobs (ledger idempotency). Reception treats the room as OCCUPIED meanwhile. |
