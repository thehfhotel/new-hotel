...
[DesignerGenerated]
public class Room_Note_Read : Office2007Form
{
	...
	[DebuggerNonUserCode]
	static Room_Note_Read()
	{
		Class2.LH6iGfYz9j3MJ();
		__ENCList = new List<WeakReference>();
	}

	public Room_Note_Read()
	{
		Class2.LH6iGfYz9j3MJ();
	...
	{
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(iHOTEL2025.Room_Note_Read));
	...
	private void Room_Note_Load(object sender, EventArgs e)
	{
		Nrows = 1;
		ISOK = false;
		DataSet dataSet = Module1.connect("select * from HT_Room_SMS where SMS_id=" + R_NO);
		RichTextBox1.Text = Conversions.ToString(dataSet.Tables[0].Rows[0]["SMS_Details"]);
		Label1.Text = Conversions.ToString(Operators.ConcatenateObject("ฝากข\u0e49อความถ\u0e36งห\u0e49อง ", dataSet.Tables[0].Rows[0]["SMS_Room"]));
		DataSet dataSet2 = Module1.connect(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("select * from HT_Room_SMS where SMS_Room='", dataSet.Tables[0].Rows[0]["SMS_Room"]), "' and SMS_Readed='no' order by SMS_ID")));
		LabelX1.Text = Conversions.ToString(Nrows);
		LabelX3.Text = Conversions.ToString(dataSet2.Tables[0].Rows.Count);
	}
	...
	private void ButtonX2_Click(object sender, EventArgs e)
	{
		ISOK = true;
		DataSet dataSet = Module1.connect("select * from HT_Room_SMS where SMS_id=" + R_NO);
		Module1.connect(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("update [HT_Room_SMS] SET [SMS_Readed]='yes' where SMS_Room='", dataSet.Tables[0].Rows[0]["SMS_Room"]), "' and SMS_Readed='no' ")));
		Close();
	}
	...
	private void ButtonX4_Click(object sender, EventArgs e)
	{
		checked
		{
			...
			DataSet dataSet = Module1.connect("select * from HT_Room_SMS where SMS_id=" + R_NO);
			DataSet dataSet2 = Module1.connect(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("select * from HT_Room_SMS where SMS_Room='", dataSet.Tables[0].Rows[0]["SMS_Room"]), "' and SMS_Readed='no' order by SMS_ID")));
			if (Nrows >= dataSet2.Tables[0].Rows.Count)
			{
				RichTextBox1.Text = Conversions.ToString(dataSet2.Tables[0].Rows[dataSet2.Tables[0].Rows.Count - 1]["SMS_Details"]);
				Nrows = dataSet2.Tables[0].Rows.Count;
			}
			else
			{
				RichTextBox1.Text = Conversions.ToString(dataSet2.Tables[0].Rows[Nrows - 1]["SMS_Details"]);
			}
			LabelX1.Text = Conversions.ToString(Nrows);
			LabelX3.Text = Conversions.ToString(dataSet2.Tables[0].Rows.Count);
		}
	...
	private void ButtonX3_Click(object sender, EventArgs e)
	{
		checked
		{
			...
			if (0 >= Nrows)
			{
				RichTextBox1.Text = Conversions.ToString(dataSet2.Tables[0].Rows[0]["SMS_Details"]);
				Nrows = 1;
			}
			else
			{
