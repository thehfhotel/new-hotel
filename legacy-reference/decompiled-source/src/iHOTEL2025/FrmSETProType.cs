...
[DesignerGenerated]
public class FrmSETProType : Office2007Form
{
	...
	[DebuggerNonUserCode]
	static FrmSETProType()
	{
		Class2.LH6iGfYz9j3MJ();
		__ENCList = new List<WeakReference>();
	}

	public FrmSETProType()
	{
		Class2.LH6iGfYz9j3MJ();
	...
	private void InitializeComponent()
	{
		...
		System.Drawing.Size size = new System.Drawing.Size(501, 369);
		groupBox2.Size = size;
		...
		location = new System.Drawing.Point(28, 333);
		...
		location = new System.Drawing.Point(212, 333);
		...
		this.ColumnHeader2.Text = "ช\u0e37\u0e48อประเภทส\u0e34นค\u0e49า";
		...
		location = new System.Drawing.Point(284, 330);
		...
		location = new System.Drawing.Point(421, 331);
	...
	public void load_type()
	{
		...
		DataSet dataSet = Module1.connect("Select * From HT_SET_ProductType order by id");
	...
	private void Button2_Click(object sender, EventArgs e)
	{
		...
		{
			MessageBox.Show("กร\u0e38ณากรอกช\u0e37\u0e48อประเภทส\u0e34นค\u0e49า");
			return;
		}
		if (Operators.CompareString(TextBox2.Text, "", TextCompare: false) == 0)
		{
			MessageBox.Show("กร\u0e38ณากรอกรห\u0e31สประเภทส\u0e34นค\u0e49า");
			return;
		}
		checked
		{
			...
			{
				if (Module1.connect("SELECT * From HT_SET_ProductType where id_full ='" + TextBox1.Text + "'").Tables[0].Rows.Count != 0)
				{
					MessageBox.Show("ม\u0e35รห\u0e31สประเภทส\u0e34นค\u0e49าน\u0e35\u0e49อย\u0e39\u0e48แล\u0e49ว");
				}
				else
				{
					object obj = "";
					obj = "INSERT INTO [HT_SET_ProductType]";
					...
					MessageBox.Show("เพ\u0e34\u0e48มช\u0e37\u0e48อประเภทส\u0e34นค\u0e49าเร\u0e35ยบร\u0e49อย");
				}
			}
			else
			{
				DataSet dataSet = Module1.connect("select * from HT_Products where Pro_no  like '" + lastcode + "-%'");
				int num = dataSet.Tables[0].Rows.Count - 1;
				int num2 = 0;
				while (true)
				{
					...
					Module1.connect(Conversions.ToString(Operators.ConcatenateObject(string.Concat("Update HT_Products SET Pro_no = '" + dataSet.Tables[0].Rows[num2]["Pro_no"].ToString().Replace(lastcode + "-", TextBox2.Text + "-"), "' where id="), dataSet.Tables[0].Rows[num2]["id"])));
					num2++;
				}
				dataSet = Module1.connect("select * from HT_Products_Price where P_id  like '" + lastcode + "-%'");
				...
				while (true)
				{
					...
					Module1.connect(Conversions.ToString(Operators.ConcatenateObject(string.Concat("Update HT_Products_Price SET P_id = '" + dataSet.Tables[0].Rows[num6]["P_id"].ToString().Replace(lastcode + "-", TextBox2.Text + "-"), "' where id="), dataSet.Tables[0].Rows[num6]["id"])));
					num6++;
				}
				Module1.connect($"Update HT_SET_ProductType SET name = '{TextBox1.Text}',id_full = '{TextBox2.Text}' WHERE id={ListView1.SelectedItems[0].SubItems[0].Text}");
	...
	public void DEL_item()
	{
		if (ListView1.SelectedItems.Count != 0)
		{
			...
			{
				DataSet dataSet = Module1.connect("select top 1 * from HT_Products where Pro_no like '" + ListView1.SelectedItems[0].SubItems[1].Text + "-%");
				if (dataSet.Tables[0].Rows.Count != 0)
				{
					MessageBox.Show("รายกาน\u0e35\u0e49ได\u0e49ถ\u0e39กใช\u0e49งานอย\u0e39\u0e48ใน รายการส\u0e34นค\u0e49า ไม\u0e48สามารถลบได\u0e49", "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand);
					return;
				}
				Module1.connect("delete from HT_SET_ProductType where id=" + ListView1.SelectedItems[0].SubItems[0].Text);
	...
	private void ToolStripMenuItem_1_Click(object sender, EventArgs e)
	{
		if (ListView1.SelectedItems.Count != 0)
		{
			...
			lastcode = ListView1.SelectedItems[0].SubItems[1].Text;
		}
		else
		{
