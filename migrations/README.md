...
| 078 | `078_loyalty_channel.sql` | Loyalty-app booking channel + membership link (`docs/loyalty-channel.md`). Adds nullable `ht_customers.cust_membership_id VARCHAR(64)` (the guest's loyalty membership id, set/cleared via `PUT /api/customers/{id}/membership` or attached by `POST /api/channel/bookings`; partial index `ix_ht_customers_membership_id`) and nullable `ht_bookings.book_hold_expires_at TIMESTAMPTZ` (payment-hold deadline for loyalty-channel TENTATIVE bookings — `book_channel='loyalty'`, `book_status='pending'`; partial index `ix_ht_bookings_hold_expiry` backs the scheduler's expiry sweep). Both columns PG-CANONICAL ONLY — no legacy `HT_Customers`/`HT_Book_H` counterpart, never written back (same policy as `cust_dob`/069 and `book_ext_ref`/076); the hold-expiry auto-cancel rides the EXISTING booking-cancel writeback path, so no new legacy write shape. ALTER-only (no new table → no CARDINALITY_MAP row; annotates the existing `ht_customers`/`ht_bookings` rows' notes). The channel API itself ships DARK behind `LOYALTY_CHANNEL_ENABLED` (default off). | vNext |
