...
export default function VerificationResults() {
  ...
  // ── HUB section (always shown) — DATA-DRIVEN per-form launcher ─────────────
  // One launch card per ACTIVE form from /api/feedback/forms (archived forms are
  // excluded server-side, so they vanish here; new named forms appear on their
  // own). Pending = no non-misclick submission matching that form's kind (+ site
  // when the form is site-scoped) yet → shown OUTSTANDING (amber + primary CTA).
  const launchForms = [...forms].sort((a, b) => a.sort - b.sort || a.key.localeCompare(b.key))
  const latestForForm = (form: FeedbackForm): VerificationResponse | null => {
    if (!responses) return null
    const wantSite = form.site && form.site !== 'all' ? form.site : null
    const rows = responses
      .filter(
        (r) =>
          asStr(r.answers?.kind) === form.kind &&
          !isMisclick(r) &&
          (wantSite ? rowSite(r) === wantSite : true),
      )
      .sort((a, b) => (new Date(b.submittedAt).getTime() || 0) - (new Date(a.submittedAt).getTime() || 0))
    return rows[0] ?? null
  }
  const formLabel = (f: FeedbackForm): string =>
    f.site && f.site !== 'all' ? `${f.title} · ${siteLabel(f.site)}` : f.title
  const pendingForms = responses ? launchForms.filter((f) => !latestForForm(f)) : []

  const hub = (
    <section className="v2-card p-4 lg:p-5 space-y-3">
      <div>
        <div className="v2-eyebrow">การตรวจสอบซ้ำ</div>
        <h2 className="text-[16px] font-semibold mt-0.5">แบบฟอร์มตรวจสอบที่เปิดใช้งาน</h2>
      </div>

      {responses && pendingForms.length > 0 && (
        <div
          className="flex items-center gap-2 rounded-lg px-3 py-2 text-[13px] font-semibold"
          style={{ color: '#b45309', background: '#fdf0d9' }}
        >
          <AlertTriangle size={15} /> ยังรอผลตรวจ {pendingForms.length} แบบฟอร์ม:{' '}
          {pendingForms.map((f) => formLabel(f)).join(', ')}
        </div>
      )}

      <div className="space-y-2">
        {launchForms.length === 0 ? (
          <p className="text-[13px]" style={{ color: 'var(--v2-ink-3)' }}>
            ยังไม่มีแบบฟอร์มที่เปิดใช้งาน
          </p>
        ) : (
          launchForms.map((f) => {
            const done = latestForForm(f)
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0 flex-wrap">
                  <span className="text-[14px] font-semibold">{formLabel(f)}</span>
                  {responses == null ? (
                    <Tag tone="mut">…</Tag>
                  ) : done ? (
                    <Tag tone="ok">
                ...
                <Link
                  href={`/v2/verification/reverify?form=${encodeURIComponent(f.key)}`}
